...
public class HeartsGUI extends JFrame implements ActionListener {
	 ...
	 * game is over
	 */
	public static final Integer EXIT_OPTION = new Integer(1);
	/** Value if the game is to be played randomly */
	 ...
	 */
	public HeartsGUI(String humanPlayerName, int seed) {
		// Set up the GUI's fields
		this.humanPlayerName = humanPlayerName;
		this.seed = seed;
		this.model = new HeartsModel(humanPlayerName, seed);
		this.status = new JLabel(model.getStatus(), JLabel.CENTER);
		this.playerLabels = new JLabel[HeartsModel.NUM_PLAYERS];
		this.humanPlayerCards = new JButton[Player.CARDS_IN_HAND];
		this.trickCards = new JLabel[HeartsModel.NUM_PLAYERS];
		...
		for (int i = 0; i < HeartsModel.NUM_PLAYERS; i++) {
	...
	public void executeEarlyCompPlayers() {
		...
		if (turn != HeartsModel.HUMAN_PLAYER) {
			// for each of those players
			for (int i = 0; i < HeartsModel.NUM_PLAYERS - turn; i++) {
				...
				status.setText(model.getStatus());
				// wait for 2 seconds
				if (i + 1 != HeartsModel.NUM_PLAYERS - turn) {
					try {
						Thread.sleep(1000);
					} catch (Exception e) {
					}
	...
	public void finishTrick(int winner) {
		...
		if (winner != HeartsModel.HUMAN_PLAYER) {
			try {
		...
		if (model.isHandFinished()) {
			...
			if (model.isGameOver()) {
				...
				if (choice.equals(EXIT_OPTION)) {
					...
					// start new game
					model = new HeartsModel(humanPlayerName, seed);
					updateStatusLabels();
					startHand();
				}
			...
			for (int i = 0; i < Player.CARDS_IN_HAND; i++) {
				...
				if (e.getSource() == humanPlayerCards[i]) {
					...
					if (model.isAcceptableMove(i)) {
						...
						trickCards[HeartsModel.HUMAN_PLAYER]
						...
						playerLabels[HeartsModel.HUMAN_PLAYER].setFont(NORMAL);
						playerLabels[HeartsModel.HUMAN_PLAYER + 1]
								.setFont(BOLD);
						status.setText(model.getStatus());
						new CPUThread().start();
						break;
	 ...
	 * played through.
	 * 
	 * @param args args[0] Player name, args[1] optional seed for Random
	 *            
	 */
	public static void main(String[] args) {
		...
		if (args.length == 2) {
			try {
				new HeartsGUI(args[0], Integer.parseInt(args[1]));
			} catch (NumberFormatException e) {
				System.out.println("USAGE: java HeartsGUI player_name seed");
			}
		} else if (args.length == 1) {
			new HeartsGUI(args[0], RANDOM_GAME);
		} else {
			System.out.println("USAGE: java HeartsGUI player_name seed");
		}
